# @patrtorg/sapiente-vitae-ut

[![NPM Package Version](https://img.shields.io/npm/v/@patrtorg/sapiente-vitae-ut)](https://www.npmjs.com/package/@patrtorg/sapiente-vitae-ut)
[![Build Status](https://github.com/patrtorg/sapiente-vitae-ut/workflows/CI/badge.svg)](https://github.com/patrtorg/sapiente-vitae-ut/actions?query=workflow%3ACI)
...
```
npm i @patrtorg/sapiente-vitae-ut
```
...
const SonicBoom = require('@patrtorg/sapiente-vitae-ut')
...
* `retryEAGAIN(err, writeBufferLen, remainingBufferLen)`: a function that will be called when @patrtorg/sapiente-vitae-ut
    write/writeSync/flushSync encounters a EAGAIN or EBUSY error. If the return value is
    true @patrtorg/sapiente-vitae-ut will retry the operation, otherwise it will bubble the
    error. `err` is the error that caused this function to be called,
    `writeBufferLen` is the length of the buffer @patrtorg/sapiente-vitae-ut tried to write, and
    `remainingBufferLen` is the length of the remaining buffer @patrtorg/sapiente-vitae-ut didn't try to write.
